...
class LongOpenLockSimEnv(gym.Env):
    ...
    def get_reward(self, info):
        self.error_evaluation_list.append(self.evaluate_error(info)) #8726.824477471144
        reward = -self.step_penalty # 1
        # x_offset
        reward += self.error_evaluation_list[-2] - self.error_evaluation_list[-1] # 483.0509776775052

        # print(f"reward part3: {reward}")
        # punish large force
        surface_diff = info["surface_diff"].clip(0.2e-3, 1.5e-3) * 1000 # info["surface_diff"]=array([9.3381e-05, 9.1564e-05]) surface_diff=[0.2,0.2]
