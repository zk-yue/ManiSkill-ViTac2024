...
class PointNetFeatureExtractor(nn.Module):
    ...
    def __init__(self, dim, out_dim, batchnorm=False):
        ...
        self.pointnet_local_fea = nn.Sequential(
            nn.Conv1d(dim, self.pointnet_local_feature_num, 1), #dim=4  self.pointnet_local_feature_num=64 
    ...
    def forward(self, marker_pos):
        ...
        :return:
        """
        if marker_pos.ndim == 3: # torch.Size([4, 4, 128, 4]) # batchsize seq_n point_num position
            marker_pos = torch.unsqueeze(marker_pos, dim=0)

        marker_pos = torch.transpose(marker_pos, 2, 3) # batchsize seq_n  position point_num torch.Size([4, 4, 4, 128])

        batch_num = marker_pos.shape[0]
        point_num = marker_pos.shape[3]

        marker_pos = torch.cat([marker_pos[:, 0, :, :], marker_pos[:, 1, :, :], marker_pos[:, 2, :, :], marker_pos[:, 3, :, :]], dim=0) # 将seq堆叠到第一维torch.Size([16, 4, 128])

        local_feature = self.pointnet_local_fea(marker_pos)  # torch.Size([16, 64, 128])
        # shape: (batch, step * 2, num_points)
        global_feature = self.pointnet_global_fea(local_feature).view(
            -1, self.pointnet_global_feature_num)  # (batch_num, self.pointnet_global_feature_num) # torch.Size([16, 512])
        ...
        pred = self.mlp_output(global_feature) # torch.Size([16, 32])
        # pred shape: (batch_num, out_dim)
        pred = torch.stack((pred[:batch_num, :], pred[batch_num:2*batch_num, :], pred[2*batch_num:3*batch_num, :], pred[3*batch_num:, :]), dim=1)
        return pred
